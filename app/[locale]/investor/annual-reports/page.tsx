...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const canonicalUrl = `${baseUrl}/${locale}/investor/annual-reports`;

  const locales = ['en', 'id', 'zh'] as const;
  const alternates = locales.map(loc => ({
    hrefLang: loc,
    href: `${baseUrl}/${loc}/investor/annual-reports`,
  }));
...
export default async function AnnualReportsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div>
      <DocumentSection section={investorPage.annualReports} locale={locale} />
