...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const canonicalUrl = `${baseUrl}/${locale}/investor/public-expose`;

  const locales = ['en', 'id', 'zh'] as const;
  const alternates = locales.map(loc => ({
    hrefLang: loc,
    href: `${baseUrl}/${loc}/investor/public-expose`,
  }));
...
export default async function PublicExposePage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const investorPage = await getInvestorPage();

  return (
    <div>
      <DocumentSection section={investorPage.publicExpose} locale={locale} bgClass="bg-muted/30" />
