...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const canonicalUrl = `${baseUrl}/${locale}/investor/financial-statements`;

  const locales = ['en', 'id', 'zh'] as const;
  const alternates = locales.map(loc => ({
    hrefLang: loc,
    href: `${baseUrl}/${loc}/investor/financial-statements`,
  }));
...
export default async function FinancialStatementsPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const investorPage = await getInvestorPage();

  return (
    <div>
      <DocumentSection section={investorPage.financialStatements} locale={locale} bgClass="bg-muted/30" />
